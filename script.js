        ...
        for (var i = 0; i < data1.length; i++) {
            ...
            col.innerHTML = `<div class="card" style="background-color: silver;" style="width: 18rem;">
              <div class="ruthra"><center> <h5 class="card-title" style="color:white;">${countryName}</h5><center></div>
                <img src="${flag}" class="card-img-top" alt="Flag" style="padding:10px;">
                <div class="card-body" ><center>
                    <p class="card-text">Capital: ${capital}</p>
                    <p class="card-text">Region: ${region}</p>
                    <p class="card-text">countrycode: ${countrycode}</p>
                    <button class="btn btn-primary" style="background-color: silver;"  onclick="click for('${countryName}')">click for Weather</button>
                    <div id="${countryName}-weather"></div></center>
                </div>
            </div>`;
